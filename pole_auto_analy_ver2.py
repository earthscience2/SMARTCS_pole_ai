...
class MainWindow(QtGui.QMainWindow):
    def __init__(self, parent=None):
        global curve1,curve2,curve3,FD_table_1,Bridge_information
        ...
        curve2 = self.login_widget.plot5.plot(pen='y') 
        curve3 = self.login_widget.plot6.plot(pen='y') 
    ...
    def pushButtonClicked_Analysis(self):
        ...
        poleid=self.login_widget.ComboBox_D_ID.currentText().split('/')[1]
        
        pole_index=int(self.login_widget.ComboBox_D_ID.currentText().split('/')[0])
   
        K_Break_location_interval=int(self.login_widget.lineEdit_limit_MAC_x.text())
        J_Sensor_location_interval=int(self.login_widget.lineEdit_limit_MAC_y.text())
        
        limit_MAC_Z=float(self.login_widget.lineEdit_limit_MAC_z.text())
        limit_MAC_X=limit_MAC_Z
        limit_MAC_Y=limit_MAC_Z
        ...
        for kk in range(1):
                ...
                stype=self.login_widget.ComboBox_D_ID.currentText().split('/')[2]
                num=int(self.login_widget.ComboBox_D_ID.currentText().split('/')[3])
                ch_index=int(self.login_widget.ComboBox_D_ID.currentText().split('/')[4])
                select_x_data,select_y_data,select_z_data,select_ch=Signal_Info_select_data_out(poleid,stype,num,limit_x,limit_y,limit_z)
                #scan_len=data['Scan_len'][pole_index]
                scan_len=1000
                print(select_ch)
                ...
                select_ch=[select_ch.index(ch_index)]
                for i in select_ch:
                    ...
                    for tt in range(10,100,10):  #시물레이션 신호 폭 변경
                        tt=tt/100
                        for k in range(25,75,K_Break_location_interval):   # 파단 위치
                           Bs=mag_m.pole_simulation(Main_mag_s,Gap_size,k,len(select_x_data[0]),scan_len*tt,HallSensor_h)
                  
                    
                           
                           for j in range(20,80,J_Sensor_location_interval):  # 센서 위치
                                ...
                                data_y = np.array([[l,w] for l,w in zip(np.gradient(Bs[:,j,1],1),select_y_data[i])])
                                data_z = np.array([[l,w] for l,w in zip(np.gradient(Bs[:,j,2],1),select_z_data[i])])
                                data_n = np.array([[l,w] for l,w in zip(abs(data_x[:,0])+abs(data_y[:,0])+abs(data_z[:,0]), 
                                                              abs(data_x[:,1])+abs(data_y[:,1])+abs(data_z[:,1]))])
                                '''
                                ...
                                phi_n, mode_scaling=normalize_phi(data_n)
                                mac_n=mac(phi_n[:,0],phi_n[:,1])
                                
                                
                                corr_x = signal.correlate(phi_n[:,0],phi_n[:,1])
                                corr_x /= np.max(corr_x)
                                corrcoef=np.corrcoef(phi_n[:,0],phi_n[:,1])[0, 1]
                                
                                max_MAC_X.append(mac_x_v)
                                max_MAC_Y.append(mac_y_v)
                                max_MAC_Z.append(mac_z_v)
                                max_MAC_N.append(mac_n)
                                
                                self.login_widget.plot7.clear()
                                self.login_widget.plot7.plot(abs(phi_n_x[:,0])+abs(phi_n_y[:,0])+abs(phi_n_z[:,0]), pen=(255,0,0)) 
                                self.login_widget.plot7.plot(phi_n[:,1]*2.4, pen=(255,255,255))
                                 
                                self.login_widget.plot9.clear()
                                self.login_widget.plot9.plot(max_MAC_X, pen=(255,0,0)) 
                                self.login_widget.plot9.plot(max_MAC_Y, pen=(0,255,0)) 
                                self.login_widget.plot9.plot(max_MAC_Z, pen=(0,0,255)) 
                                
                                mac_x_v=round(max(max_MAC_X, key=abs),3)
                                mac_y_v=round(max(max_MAC_Y, key=abs),3)
                                mac_z_v=round(max(max_MAC_Z, key=abs),3)
                                mac_n=round(max(max_MAC_N, key=abs),3)
                                
                                new_data={'poleid':poleid,'s_num':num,'stype':stype, 'simu_rate':tt,'X_location':k,'CH':ch_index,'S_position':j,'mac_x_v':round(mac_x_v,3),'mac_y_v':round(mac_y_v,3),'mac_z_v':round(mac_z_v,3),'corrcoef':round(corrcoef,3)}
                                          #"S_x":phi_n_x[:,0],"S_y":phi_n_y[:,0],"S_z":phi_n_z[:,0]}
                                
                                                                
                                self.login_widget.label_MAC.setText('MAC : '+str(mac_x_v)+'/'+str(mac_y_v)+'/'+str(mac_z_v)+'/'+str(mac_n))
                                ...
                                if mac_x_v > limit_MAC_X and mac_y_v > limit_MAC_Y and mac_z_v > limit_MAC_Z and abs(corrcoef)>limit_corrcoef:   
                                   ...
                                   #self.login_widget.plot7.plot(phi_n_x[:,1]+phi_n_y[:,1]+phi_n_z[:,1], pen=(255,0,0)) 
                                   self.login_widget.plot8.plot(phi_n_x[:,1], pen=(255,0,0))
                                   self.login_widget.plot8.plot(phi_n_y[:,1], pen=(0,0,255))
                                   self.login_widget.plot8.plot(phi_n_z[:,1], pen=(0,255,0)) 
    ...
    def pushButtonClicked_file_open(self):
        ...
        data=Scan_data.loc[Scan_data['prediction']=='NG'].reset_index().drop_duplicates(subset=None, keep='first', inplace=False, ignore_index=False)
        
        
        
        pole_Item_list=[]
        for index, item in enumerate(data['poleid']):
            
            aa=str(index)+'/'+ item+'/'+data['stype'][index]+'/'+str(int(data['s_num'][index]))+'/'+str(int(data['CH'][index]))
            pole_Item_list.append(aa)
            
        
        self.login_widget.ComboBox_D_ID.clear()
        self.login_widget.label_Analysis_pole_total_num.setText(str(len(data['poleid'])))
        self.login_widget.ComboBox_D_ID.addItems(pole_Item_list) 
...
class LoginWidget(QtGui.QWidget):
    def __init__(self, parent=None):
        ...
        self.label_limit_MAC_x =  QtGui.QLabel('K_Break_location_interval')
        self.lineEdit_limit_MAC_x=  QtGui.QLineEdit('5')
        
        
        self.label_limit_MAC_y =  QtGui.QLabel('J_Sensor_location_interval')
        self.lineEdit_limit_MAC_y=  QtGui.QLineEdit('5')
