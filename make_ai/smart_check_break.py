...
def check_csv():
    ...
    for server in server_list:
        ...
        # 파단 데이터 수집
        print(server + " 서버 // 추가 파단 데이터 찾는 중....")
        for group_name in tqdm(groupname_list, desc="Processing poles"):
            x_files = glob.glob(f"raw_data_ai_csv/break/{group_name}/*_H_x.csv")
            y_files = glob.glob(f"raw_data_ai_csv/break/{group_name}/*_H_y.csv")
            z_files = glob.glob(f"raw_data_ai_csv/break/{group_name}/*_H_z.csv")
            
            json_file_path = (f"raw_data_ai_json/break/{group_name}.json")
                ...
                for x_file, y_file, z_file in zip(x_files, y_files, z_files):
                    ...
                    for i in break_data:
                        if not (os.path.exists("raw_data_ai_csv_check/break/" + file_name + ".csv") or os.path.exists("raw_data_ai_csv_check/normal/" + file_name + ".csv") or os.path.exists("raw_data_ai_csv_check/other/" + file_name + ".csv")):
                            if i[0] == pole_name:
                                stdegree = float(file_name_list[-6])
                                eddegree = float(file_name_list[-5])
                                stheight = float(file_name_list[-4])
                                edheight = float(file_name_list[-3])
                                if stdegree <= float(i[2]) <= eddegree and stheight <= float(i[1]) <= edheight:
                                    ...
                                    stand = (float(i[1]) - stheight) / (edheight - stheight)
                                    start_index = max(0, int(total_rows * (stand - 0.10))) 
                                    end_index = min(total_rows, int(total_rows * (stand + 0.10)))
                                    x_data = x_data[start_index:end_index]
                                    y_data = y_data[start_index:end_index]
                                    z_data = z_data[start_index:end_index]
                                    ...
                                    if user_input == 'y':
                                        if not (x_data.empty or y_data.empty or z_data.empty):
                                            ...
                                            combined_data['label'] = 1  # 파단 데이터 레이블
                                            combined_data.to_csv(os.path.join("raw_data_ai_csv_check/break", file_name + ".csv"), index=False)
                                    elif user_input == 'n':
                                        if not (x_data.empty or y_data.empty or z_data.empty):
                                            combined_data = pd.concat([x_data, y_data, z_data], axis=1)
                                            combined_data = combined_data.drop(columns=['x_idx','x_measno','x_poleid','x_groupname','x_devicetype','x_axis','y_idx','y_measno','y_poleid','y_groupname','y_devicetype','y_axis','z_idx', 'z_measno','z_poleid','z_groupname','z_devicetype','z_axis'])
                                            combined_data['label'] = 2  # 비파단 데이터 레이블
