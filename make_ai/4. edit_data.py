...
def process_out_files_for_measno(pole_dir, measno, meas_info, output_dir):
    ...
    # 찾은 파일 정보 로그 출력 (디버깅용)
    print(f"  [{poleid}] measno {measno}: 찾은 파일 - X: {os.path.basename(file_x)}, Y: {os.path.basename(file_y)}, Z: {os.path.basename(file_z)}")
...
def process_pole_directory(pole_dir, output_dir):
    ...
    # 파단 데이터이고 파일이 생성된 경우 break_info.json 저장
    ...
    if processed_count > 0 and is_break_info and breakstate == 'B':
        ...
        output_info_file = os.path.join(output_subdir, f"{poleid}_break_info.json")
        
        try:
            with open(output_info_file, 'w', encoding='utf-8') as f:
                json.dump(break_info, f, ensure_ascii=False, indent=2)
        except Exception as e:
            pass  # 저장 실패해도 무시
...
def process_all_raw_pole_data(
    ...
    for data_type in ['break', 'normal']:
        ...
        for project_dir in sorted(projects):
            ...
            for pole_dir in tqdm(sorted(pole_dirs), desc=f"  {project_name} 처리 중"):
                total_poles += 1
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="raw_pole_data의 OUT 파일들을 x, y, z로 병합하고 height, degree 그리드로 변환"
    )
    parser.add_argument(
        "--input-dir",
        type=str,
        default="3. raw_pole_data",
        help="원본 데이터 기본 디렉토리",
    )
    parser.add_argument(
        "--output-dir",
    ...
    process_all_raw_pole_data(
        raw_data_base_dir=args.input_dir,
