...
# -*- coding: utf-8 -*-
"""
3. raw_pole_data 디렉토리의 정보를 확인하는 스크립트
- 프로젝트 수
- 파일 수 (CSV, JSON)
- 파단 전주 수
- 정상 전주 수
...
def count_files_in_directory(directory):
    ...
    if not directory.exists():
        return csv_count, json_count, other_count
...
def analyze_raw_pole_data():
    """raw_pole_data 디렉토리 분석"""
    if not raw_data_dir.exists():
        print(f"오류: 디렉토리를 찾을 수 없습니다: {raw_data_dir}")
        return
    
    print("=" * 80)
    print("3. raw_pole_data 디렉토리 정보 분석")
    print("=" * 80)
    print()
    ...
    print("=" * 80)
    print()
    print(f"{'구분':<15} {'프로젝트 수':<15} {'전주 수':<15} {'CSV 파일 수':<20} {'JSON 파일 수':<20}")
    print("-" * 80)
    print(f"{'파단':<15} {len(total_stats['break']['projects']):<15} {total_stats['break']['poles']:<15} {total_stats['break']['csv_files']:<20,} {total_stats['break']['json_files']:<20,}")
    print(f"{'정상':<15} {len(total_stats['normal']['projects']):<15} {total_stats['normal']['poles']:<15} {total_stats['normal']['csv_files']:<20,} {total_stats['normal']['json_files']:<20,}")
    print(f"{'전체':<15} {len(all_projects):<15} {total_poles:<15} {total_csv:<20,} {total_json:<20,}")
    print()
