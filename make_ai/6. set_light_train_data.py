...
def collect_break_files_from_edit_data(
    ...
    edit_break = CURRENT_DIR / edit_data_dir / "break"
    merge_break = CURRENT_DIR / merge_data_dir / "break"
...
def collect_all_crop_files(data_dir: str, is_break: bool) -> List[Tuple[str, str, str, int]]:
    ...
    base = CURRENT_DIR / data_dir / ("break" if is_break else "normal")
    result: List[Tuple[str, str, str, int]] = []
    if not base.exists():
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default="6. light_train_data", help="출력 디렉터리")
